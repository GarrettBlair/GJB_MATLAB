...
% ===================
clear
cd('C:\Users\gjb326\Documents\MATLAB\DLAG-master')
...
%%%%%%%%%%%%%%%%%%% dlag_testdata;
 % USE
 dlag_testdata
 % C:\Users\gjb326\Documents\MATLAB\GJB_MATLAB\dlag_scripts\dlag_testdata.m
 % to setup data
%% =======================
...
% GJB 2024 see also https://github.com/egokcen/DLAG/wiki/Speedup-Options
% =====================
% 1a) Fitting a DLAG model
% ========================
%   first run -  dlag_testdata.m
% Let's explicitly define all of the optional arguments, for 
% the sake of demonstration:
runIdx = 101;               % Results will be saved in baseDir/mat_results/runXXX/,  
% 101: dlag on my data, split trails
% 102: dlag example data full output
% 103: dlag my data, no split
                          % where XXX is runIdx. Use a new runIdx for each dataset.
baseDir = 'C:\Users\gjb326\Documents\MATLAB\DLAG-master\OUTPUT\'; % Base directory where results will be saved
...
binWidth = binWidth;            % Sample period / spike count bin width, in units of time (e.g., ms)
numFolds = 0;%5;             % Number of cross-validation folds (0 means no cross-validation)
...
yDims = [ns ns];%[10 51];          % Number of observed features (neurons) in each group (area)
rGroups = [1 2];          % For performance evaluation, we can regress group 2's activity with group 1
startTau = 5*binWidth;%2*binWidth;    % Initial timescale, in the same units of time as binWidth
trial_length = seqTrue(1).T;
segLength = trial_length; % Largest trial segment length, in no. of time points
...
maxIters = 5e3;%5e3           % Limit the number of EM iterations (not recommended for final fitting stage)
...
fit_dlag(runIdx, seqTrue, ...
         ...
         'parallelize', true, ... % Only relevant for cross-validation
...
yest = zeros(12, size(spks, 2));
for i = 1:length(seqEst)
    yest(:, (i-1)*trial_length + 1:(i)*trial_length) = seqEst(i).xsm;%y;
end
% yest = yest(:, 1:size(spks,2));
yest = yest(:, 2:end-1);
