function [struct_in] = construct_place_maps_1D(struct_in, x, dt, spks, bins, params)
...
nbins = length(bins)-1;
[vmap, ~]   = make_occupancymap_1D(x, dt, bins);
% vmap(vmap<occupancy_thresh) = NaN;
nsegs = size(spks, 1);
spkmap = NaN(nsegs, nbins);
pfields = NaN(nsegs, nbins);
pfields_smooth = NaN(nsegs, nbins);
...
for i = 1:nsegs
    ...
    [smap, ~] = make_occupancymap_1D(x,  spks(i, :), bins);
    ...
    % smoothing place map
    smap(isnan(smap)) = 0;
    sm_smap = conv(smap, smoothing_kern, 'same');
