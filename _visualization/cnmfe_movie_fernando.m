...
frames = 1:2:1000; % frames to display
...
cmap = jet(length(segs));
[~, ord] = sort(rand(length(segs),1));
cmap = cmap(ord,:);
...
figure;
set(gcf, 'Color', 'k', 'Position', [300 300 400 400]) % change this to change the output size
time_win = 30; % number of samples pre and post to plot
...
for i = 1+time_win:length(frames)-time_win
    ...
    subplot(2,2,1);
    colormap gray
    imagesc(im, [30 180]) % change this scaling if needed to see raw activity, between 0 and 255
    axis image off
