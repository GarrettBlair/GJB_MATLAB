...
if ishandle(progfig)
    ...
    for ndx = 1:nbars
        ...
        changecolor([], [], progdata(ndx).progpatch, 100*ndx/(nbars+1))
...
lastupdate = clock;
% ------------------------------------------------------------------------------
function changecolor(h, e, progpatch, ndx) %#ok<INUSL>
...
try
all_colors = plasma(100);
catch
all_colors = jet(100);
end
all_colors = all_colors./2 + .4; % 
% while (sum(thiscolor) < colormin) || (sum(thiscolor) > colormax)
%     thiscolor = rand(1, 3);
% end
thiscolor = all_colors(ndx,:);
