...
behavior_dir = 'sleap behavior output\@SMARTies';
...
for animalLoop = 1:numAnimals
    ...
    for cameraLoop = 1:length(params.cameraName)
        ...
        for sessionLoop = 1:AnimalDir.numSess
            ...
            sleapfile_input  = sprintf('%s%s/%s_%s_%s.mat', experiment_folder, animal_name, behavior_dir, animal_name, AnimalDir.SessType{sessionLoop});
...
%                     [ms, behav, params_sub] = APA_generate_data_struct(AnimalDir, sessionLoop, params);
                    behav = load(sleapfile_input);

                    [ms, ~, params_sub]     = SMART_generate_data_struct(AnimalDir, sessionLoop, behav.room, behav.arena, params);
                    ms.params               = params_sub;
                    ms.cameraName           = cameraName;
                    [ms]                    = extract_caiman_data(ms, params, cameraName);
                    ms.sleap.fileName       = sleapfile_input;
                    ms.room.DAT_fileinfo    = behav.room.DAT_fileinfo;
                    ms.room.timestamps      = ms.timestamps;
                    ms.arena.DAT_fileinfo   = behav.arena.DAT_fileinfo;
                    ms.arena.timestamps     = ms.timestamps;
                    
                    params.strict_errors_only = false;
                    params.during_rotation_only   = false;
                    [ms.zonestruct] = DUAL8MAZE_performance_eval(ms.room, ms.arena, params, false);
%                     correct_linvec = ms.zonestruct.choices.paths.linear_vec;
%                     correct_linvec(ms.zonestruct.choices.paths.error==true) = correct_linvec(ms.zonestruct.choices.paths.error==true)*-1;
%                     left_linvec = ms.zonestruct.choices.paths.linear_vec;
%                     left_linvec(ms.zonestruct.choices.paths.right>0)    = left_linvec(ms.zonestruct.choices.paths.right>0)*-1;
%                     figure(sessionLoop); hold on
%                     plot(ms.timestamps, correct_linvec)
%                     plot(ms.timestamps, left_linvec)
        ...
        for sessionLoop = 1:AnimalDir.numSess
            ...
            if isempty(dir(processedFile)) == true || resave_proccessed==true
                ...
                if PRINT_ONLY==false && isfile(placecellFile)
                    ...
                    save(processedFile, 'ms', 'behav', 'params', 'analysis_version', 'AnimalDir', '-v7.3');
                end
                %
                fprintf(' Done!\n\t\t%s\n', processedFile)
            else
...
vars2save = {'parentDir' 'filename' 'time_ms' 'dt_ms' 'roomx' 'roomy' 'arenax' 'arenay'...
    ...
    'left_linvec' 'correct_linvec' 'lapsPerMin' 'p_left' 'p_correct'};
...
p_correct = ms.zonestruct.metrics.p_correct;
p_left = ms.zonestruct.metrics.p_left;
lapsPerMin = ms.zonestruct.metrics.lapsPerMin;
