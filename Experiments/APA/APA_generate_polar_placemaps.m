function [ms] = APA_generate_polar_placemaps(ms, params)
...
[rtheta, rrho] = cart2pol(ms.room.x, ms.room.y);
[atheta, arho] = cart2pol(ms.arena.x, ms.arena.y);

ms.room.split_vec = [];
ms.arena.split_vec = [];
[ms.room]       = construct_place_maps_2Dpolar(ms.room,  rtheta(is_moving),  rrho(is_moving), ms.dt(is_moving), spks(:, is_moving), params.yaw_bins, params.rho_bins, params);
[ms.arena]      = construct_place_maps_2Dpolar(ms.arena, atheta(is_moving),  arho(is_moving), ms.dt(is_moving), spks(:, is_moving), params.yaw_bins, params.rho_bins, params);
...
[ms.room.pfield_decode]  = pfield_split_bayesian_decoding(ms, rtheta,  rrho, spks, params.yaw_bins, params.rho_bins, 1:size(spks,1), params.num_random_shuffle_decode, params.ipos_int_time);
[ms.arena.pfield_decode] = pfield_split_bayesian_decoding(ms, atheta,  arho, spks, params.yaw_bins, params.rho_bins, 1:size(spks,1), params.num_random_shuffle_decode, params.ipos_int_time);
