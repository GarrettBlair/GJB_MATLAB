...
dt = abs(diff(temp_ts));
[ms.room]  = construct_place_maps_2D(ms.room,  ms.room.x(is_moving),  ms.room.y(is_moving),  dt(is_moving), spks(:, is_moving), params.pos_bins, params);
[ms.arena] = construct_place_maps_2D(ms.arena, ms.arena.x(is_moving), ms.arena.y(is_moving), dt(is_moving), spks(:, is_moving), params.pos_bins, params);
...
[ori_str] = construct_place_maps_1D(ori_str, ori_str.yaw(is_moving), ms.dt(is_moving), spks(:, is_moving), yaw_bin, params);
figure; imagesc(ori_str.pfields_smooth)
