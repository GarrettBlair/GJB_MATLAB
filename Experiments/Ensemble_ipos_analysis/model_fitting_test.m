...
cents = edges_of_bins(1:end-1) + bin_size;
...
model_out=lognpdf(bin_center,centroid_distances_model_parameters(2),centroid_distances_model_parameters(3));
model_out=model_out./sum(model_out)*(number_of_bins/((bin_center(2)-bin_center(1))+microns_per_pixel*(bin_center(end)-bin_center(1))));
...
% calculating the distribution for same cells:
model_resid=lognpdf(bin_center,centroid_distances_model_parameters(2),centroid_distances_model_parameters(3));
model_resid=model_resid./sum(model_resid)*(number_of_bins/((bin_center(2)-bin_center(1))+microns_per_pixel*(bin_center(end)-bin_center(1))));

model_intersect = model_resid.*model_out;%./(sum(model_resid.*model_out));
model_product = model_resid*model_out';
subplot(2,1,1); hold on
plot(bin_center, data_dist, 'k')
plot(bin_center, model_out, 'b')
plot(bin_center, model_resid, 'r-')
yyaxis('right')
plot(bin_center, model_intersect, 'm:')

subplot(2,1,2); hold on
plot(bin_center, cumsum(model_out), 'b')
plot(bin_center, cumsum(model_resid), 'r')

yyaxis('right')
plot(bin_center, cumsum(model_intersect), 'm:')
title(sprintf('product = %1.3f', model_product))
