clear
load('D:\Sample Data\ensemble_prob_openfield_test\standard\256_101.mat') % data with pobabilitie vectors saved
w = ~isnan(ensem_av1);
figure(2); clf; 
...
plot(probs_ps_x1(w)); 
plot(probs_px1(w)); 
plot(switch_binned(w), 'r')
legend({'p(s)', 'p(s\_x)', 'p(x)'})
axis tight
...
histogram(probs_ps1(switch_binned==0), [0:.025:1]);%, 'Normalization', 'probability');
histogram(probs_ps1(switch_binned==1), [0:.025:1]);%, 'Normalization', 'probability');
title('p(s)')
subplot(3,1,2); hold on
histogram(probs_ps_x1(switch_binned==0), [0:.025:1]);%, 'Normalization', 'probability');
histogram(probs_ps_x1(switch_binned==1), [0:.025:1]);%, 'Normalization', 'probability');
title('p(s\_x)')
subplot(3,1,3); hold on
histogram(probs_px1(switch_binned==0), [0:.025:1]);%, 'Normalization', 'probability');
histogram(probs_px1(switch_binned==1), [0:.025:1]);%, 'Normalization', 'probability');
title('p(x)')
...
nc=2; nr=2;
figure(3011); clf;
set(gcf, 'Units', 'inches', 'OuterPosition', [1   1   6   6])%, 'PaperSize', [1,1])
...
axis([-.6 .6 -.6 .6])
axis square
subplot_tight(nr*12,nc,1, [.001 .075]); cla; hold on
...
subplot_tight(nr,nc,2, [.075 .075]); hold on
hold on
...
%
subplot_tight(nr,nc,3, [.075 .075]); hold on
hold on
...
subplot_tight(nr,nc,4, [.075 .075]); hold on
hold on
scatter(iso(1,~idx), iso(2,~idx), 30, [.7 .7 .7], '.', 'MarkerFaceAlpha', .2)
...
nc=2; nr=3;
figure(3021); clf;
set(gcf, 'Units', 'inches', 'OuterPosition', [4   1   6   8])%, 'PaperSize', [1,1])
subplot_tight(nr,nc,1, [.075 .075]); hold on
ind_all = find(xx1>0);
...
subplot_tight(nr,nc,5:6, [.15 .15]); hold on
...
%
figure(3031); clf;
set(gcf, 'Units', 'inches', 'OuterPosition', [7   1   6   8])%, 'PaperSize', [1,1])
subplot_tight(nr,nc,1, [.075 .075]); hold on
