figure(5); clf
all_animal_name = {'Hipp16942'};%animals{animalLoop};
...
if true%strcmp(animal_name, 'Hipp16942')
AnimalDir.processedFile = AnimalDir.processedFile(4:end);
matchingfname = sprintf('%s%s/2022_06_20_matching.mat', experiment_folder, animal_name);
% cellmap = cellmap(:,5:9);
numsess = size(AnimalDir.processedFile,1);
sess_corr1 = NaN(numsess);
% subplot(1,3,1)
colors = viridis(numsess*3);
colors = jet(numsess);%colors(6:10,:);
light_colors = colors + (1-colors)/3;
else
AnimalDir.processedFile = AnimalDir.processedFile(1:3);
matchingfname = sprintf('%s%s/2022_06_10_matching.mat', experiment_folder, animal_name);
colors = jet(numsess);%colors(6:10,:);
end
load(matchingfname);
%%
...
numcells2use = [];%80;
sess_corr = NaN(numsess);
pall{aLoop} = NaN(numsess,15);
for s1 = 1:numsess
%     s2 = s1;
    for s2 = s1:numsess
        if s1~=s2
...
%             sum(matched)
            ...
            [popcorr1, ~, ~] = Fenton_pop_stability(spks1, pop_dt1, false);
            [popcorr2, ~, ~] = Fenton_pop_stability(spks2, pop_dt2, false);
            ...
            spks1 = f1.ms.neuron.S_matw(cellmap(matched,s1), :);
%             craw = normalize_rows(f1.ms.neuron.C(cellmap(matched,s1),:)+f1.ms.neuron.YrA(cellmap(matched,s1),:));
...
if strcmp(animal_name, 'Hipp16942')
...
a = sess_corr1(~isnan(sess_corr1)); b = a;%sess_corr2(~isnan(sess_corr2));
subplot(1,2,2); cla
hold on
