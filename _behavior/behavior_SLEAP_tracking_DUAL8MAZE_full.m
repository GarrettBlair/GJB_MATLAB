function [room, arena, ZoneStruct, params] = behavior_SLEAP_tracking_DUAL8MAZE(room_tracking_fname, arena_tracking_fname, sleap_fname, sleap_timestamp_fname, params)
% global params
debug = true;
...
if ~isempty(sleap_fname)
    ...
    ledx = 580*ones(length(ax),1); ledy =240*ones(length(ax),1);
...
%         room.timestamps = sleap_ts*1000;
%         arena.timestamps = sleap_ts*1000;
    end
...
%     n_edges = length(sleap.edge_names);
    ...
    [at, arho] = cart2pol(mxy(:,1)-video_center(2), mxy(:,2)-video_center(1)); 
    
    at = at+angdiff1;
    [ax2, ay2] = pol2cart(at, arho);
    arena.rawx = ax2+video_center(2);
    arena.rawy = ay2+video_center(1);
    
    [at, arho] = cart2pol(ledx-video_center(2), ledy-video_center(1)); 
    at = at+angdiff1;
    [ax2, ay2] = pol2cart(at, arho);
    ledx_room = ax2+video_center(2);
    ledy_room = ay2+video_center(1);
...
%         y = (y-params.arena_center(2))./params.pixpercm;
        x = (x-video_center(2))./video_pixpercm;
        y = (y-video_center(1))./video_pixpercm;
...
%     subplot(121); hold on
%     plot3(x(a:b), y(a:b), ts(a:b), 'b')
%     subplot(122); hold on
%     plot3(x(a:b), y(a:b), ts(a:b), 'Color', cc)
end
for ind = 1:idxR
    a = R_start(ind);
    b = R_end(ind);
...
figure; hold on;
plot3(x,y,ts)
% plot3(x(R_start) ,y(R_start),ts(R_start), 'ro')
...
% yyaxis('right');
% plot(isTime/60, isLeft, 'k')
% plot(isTime/60, isCorrect, 'g')
...
if isfield(arena, 'ArenaTrackReinforcedSector')
