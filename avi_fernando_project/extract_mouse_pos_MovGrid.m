function extract_mouse_pos_MovGrid(topdir, saveName, cropFlag, params)
cropX = params.cropX;
cropY = params.cropY;
lastx = params.lastX;
lasty = params.lastY;
lastfnum = params.lastfnum;
...
vars2save = {'beh', 'topdir', 'mouse', 'led', 'barpos', 'cropX', 'cropY', 'fullframe_cropX', 'fullframe_cropY'};
%%
...
%             im = uint8(vid.read(1));
            ...
            title('Crop the full frame, click 2 opposing corners')
            [fullframe_cropX, fullframe_cropY] = ginput(2);
            fullframe_cropX = round([min(fullframe_cropX) max(fullframe_cropX)]);
            fullframe_cropY = round([min(fullframe_cropY) max(fullframe_cropY)]);
else
    fullframe_cropX = [1 size(im, 2)];
    fullframe_cropY = [1 size(im, 1)];
end
            
height = 1+fullframe_cropY(2)-fullframe_cropY(1);
width = 1+fullframe_cropX(2)-fullframe_cropX(1);
nframe = beh.frameNum(end);
Yr = uint8(false(height, width, beh.frameNum(end)));
Yg = uint8(false(height, width, beh.frameNum(end)));
Yb = uint8(false(height, width, beh.frameNum(end)));
Yrbg = uint8(false(height, width, beh.frameNum(end)));
Ygbg = uint8(false(height, width, beh.frameNum(end)));
Ybbg = uint8(false(height, width, beh.frameNum(end)));
step = 1;
fprintf('\n\tLoading %d behavior videos...\n', numVids)
for i = 1:numVids
...
%     for ind = 1:step:frames2read
        ind = ind+1;
        fnum = (i-1)*1000 + ind;
        im = uint8(vid.readFrame);
%         im = uint8(vid.read(ind));
        im = im(fullframe_cropY(1):fullframe_cropY(2), fullframe_cropX(1):fullframe_cropX(2),:);
        Yr(:,:,fnum) = im(:,:,1);
        Yg(:,:,fnum) = im(:,:,2);
        Yb(:,:,fnum) = im(:,:,3);
        ...
        bgind = fnum-2;
        if bgind>0
            bgwin = bgind-2:bgind+2;
            bgwin = bgwin(bgwin>0 & bgwin<=fnum);
            Yrbg(:,:,bgind) = uint8(mean(Yr(:,:,bgwin), 3));
            Ygbg(:,:,bgind) = uint8(mean(Yg(:,:,bgwin), 3));
            Ybbg(:,:,bgind) = uint8(mean(Yb(:,:,bgwin), 3));
        end
        lastf = fnum;
    end
end
...
Ygbg = Yg(:,:,1:lastf);
Ybbg = Yb(:,:,1:lastf);
% parfor ind = bgind+1:lastf
%     bgwin = ind-2:ind+2;
%     bgwin = bgwin(bgwin>0 & bgwin<=lastf);
%     r = Yr(:,:,bgwin);
%     Yrbg(:,:,ind) = uint8(mean(r, 3));
%     Ygbg(:,:,ind) = uint8(mean(Yg(:,:,bgwin), 3));
%     Ybbg(:,:,ind) = uint8(mean(Yb(:,:,bgwin), 3));
% end
fprintf('Caculating background\n')
% % Yrbg = uint8(movmean(Yr, 5, 3));
% % Ygbg = uint8(movmean(Yg, 5, 3)); % 30
% % Ybbg = uint8(movmean(Yb, 5, 3));
...
%%
% MOUSE PARAMS
mouse_sz = 8; % radius pixels
minmousearea = 100; % area, pixels
beh_sooth_sz = 7; beh_halfsz = ceil(beh_sooth_sz/2);

% MOVING GRID PARAMS
minbararea = 60; % area, pixels
% bar_thresh = 230; % pixel value thresh for bar on frame mean (rgb); used for initial cohorts
bar_thresh = 180; % pixel value thresh for bar on frame mean (rgb); used for noshock group
grid_sz = [80 40]; % h x w pixels for morph operation

% MS LED PARAMS
led_sizemin = 4; % min area for LED, pixels
led_sizemax = 75; % max area for LED, pixels
searchsize = 75; % window size for subplot, cropX
minLEDval = 200; % inimum vlue for the max pixel in ROI

led_sz =3; % radius in pix

LED_se = strel('disk', led_sz);
smallgauss2d = gausswin(led_sz*2 - 1, 1); smallgauss2d  = smallgauss2d*smallgauss2d'; smallgauss2d = smallgauss2d./sum(smallgauss2d(:));
mouse_se = strel('disk', mouse_sz);
grid_se = strel('rectangle', grid_sz);
line = strel('rectangle', [3,1]);


mouse_x = NaN(numVids*1000,1);
mouse_y = NaN(numVids*1000,1);
mouse_ecc = NaN(numVids*1000,1);
mouse_size = NaN(numVids*1000,1);
barpos = NaN(numVids*1000,1);
...
tic
step = 1; % 1;
plotstep = 1:100:numVids*1000;
plotstepreport = 1:5000:numVids*1000 - 1;
...
% vo.FrameRate = 30;
% vo.open();
fprintf('\n\tEvaluating mouse and bar position...\n')
debug = false;

parfor fnum = 1:step:lastf
    %%
...
%     end
    ...
    imbar = imbar(cropY(1):cropY(2), cropX(1):cropX(2));
    ...
    immouse = conv2(immouse, smallgauss2d, 'same');
    rdiffmouse = -immouse; % imr - immouse;
    rdiff_mouse = rdiffmouse >= quantile(rdiffmouse(:), .995) & rdiffmouse<quantile(imr(:), .5);
    
    rdiffmouse = rdiffmouse(cropY(1):cropY(2), cropX(1):cropX(2));
    rdiff_mouse = rdiff_mouse(cropY(1):cropY(2), cropX(1):cropX(2));
...
xs(1:beh_halfsz) = x(1:beh_halfsz); xs(end-beh_halfsz:end) = x(end-beh_halfsz:end); 
ys(1:beh_halfsz) = y(1:beh_halfsz); ys(end-beh_halfsz:end) = y(end-beh_halfsz:end); 
speed = speed_calc_gb(xs,ys); 

mouse = [];
mouse.x = xs;
mouse.y = ys;
mouse.rawx = mouse_x(1:lastf);
mouse.rawy = mouse_y(1:lastf);
mouse.speed = speed;
mouse.area = mouse_size(1:lastf);
mouse.ecc = mouse_ecc(1:lastf);
%%
plotting=false;
tic
step = 1;
...
led_x = NaN(numVids*1000,1);
led_y = NaN(numVids*1000,1);
led_meanval = NaN(numVids*1000,1);
led_maxval = NaN(numVids*1000,1);
led_dist = NaN(numVids*1000,1);
led_size = NaN(numVids*1000,1);
led_angdiff = NaN(numVids*1000,1);
headangle = NaN(numVids*1000,1);
...
futurewindow = 15; % # of frames to look ahead for path
posdistthresh= 100;% distance in cropX to look towards
...
disp('Calculating miniscope LED position')
for fnum =  1:step:lastf % 17365:step:17537
    ...
    % imbar = imbar(cropY(1):cropY(2), cropX(1):cropX(2));
    if isempty(lastx)
        [lastx, lasty, lastfnum] = fernando_helper_click_mini(im, cropX, cropY);
    end
    ...
    im2 = im(cropY(1):cropY(2), cropX(1):cropX(2),1);
    ...
    ledmap = ledmap./max(ledmap(:));


    % pathmap = pathmap+ledmap + ledmap;
    pathmap = pathmap + futuremap + ledmap;
    ...
    d = imLED > 5000 & ~imbar;
    validLED=d(cropY(1):cropY(2), cropX(1):cropX(2)).*validmap;
    validLED = validLED>0;
    imLED = imLED(cropY(1):cropY(2), cropX(1):cropX(2));
        imLED2 = normalize_matrix(imLED);
    if showLED==true && debug
        if isbg
        imav2 = normalize_matrix(imavbg(cropY(1):cropY(2), cropX(1):cropX(2)));
        else
        imav2 = normalize_matrix(imav(cropY(1):cropY(2), cropX(1):cropX(2)));
        end
        figure(111); clf; set(gcf, 'Position', [163   412   560   189])
        imagesc([imav2; imLED2; validLED]);
        title(['bg is ' num2str(1*isbg)])
    end
        imav=imav(cropY(1):cropY(2), cropX(1):cropX(2)).*validmap;
        imb=imb(cropY(1):cropY(2), cropX(1):cropX(2)).*validmap;
        imr=imr(cropY(1):cropY(2), cropX(1):cropX(2)).*validmap;
    ...
    if debug && any(ismember(fnum, plotstep))
        ...
        subplot_tight(3,1,1)
        imagesc(imLED2, [.2 .5]); axis image; colormap(cmap)
%         imagesc(imLED2, [0 1]); axis image; colormap(cmap)
        ...
        subplot_tight(3,1,3)
        cla
        imbar2 = imbar(cropY(1):cropY(2), cropX(1):cropX(2));
...
lxs = conv(lx, smallgauss1d, 'same');
lys = conv(ly, smallgauss1d, 'same');
lhs = conv(lh, smallgauss1d, 'same');
lxs(1:beh_halfsz) = lx(1:beh_halfsz); lxs(end-beh_halfsz:end) = lx(end-beh_halfsz:end); 
lys(1:beh_halfsz) = ly(1:beh_halfsz); lys(end-beh_halfsz:end) = ly(end-beh_halfsz:end); 
lhs(1:beh_halfsz) = lh(1:beh_halfsz); lhs(end-beh_halfsz:end) = lh(end-beh_halfsz:end); 

naninds = isnan(lxs)& isnan(lys);
lxs(naninds)= interp1(find(~naninds), lxs(~naninds), find(naninds), 'nearest', 'extrap');
lys(naninds)= interp1(find(~naninds), lys(~naninds), find(naninds), 'nearest', 'extrap');
naninds = isnan(lh);
lhs(naninds)= interp1(find(~naninds), lhs(~naninds), find(naninds), 'nearest', 'extrap');



spd = speed_calc_gb(lxs, lys);
...
led.rawy = ly;
led.speed = spd;
led.meanVal = led_meanval(1:lastf);
led.maxVal = led_maxval(1:lastf);
led.dist = led_dist(1:lastf);
led.area = led_size(1:lastf);
led.angdiff = led_angdiff(1:lastf);

%% speed = speed_calc_gb(lxs,lys); 
if true
%%
vo = VideoWriter('C:\Users\gjb326\Videos\data_demos\noshock_mouse5.avi');
vo.open()
searchsize = 45; % 75;
...
startf = 1;
LED_se = linspace(.7, .3, lastf-startf+1);
lasty = [.8 .7 .9]*.85;
...
%         imL = im2; imL = imL+.8*(255-imL);
        ...
        lh = plot(xs(startf:fnum), ys(startf:fnum), '-', 'Color', lasty, 'LineWidth', 1.5);
        lh.Color=[lasty, LED_se(fnum)];
        ...
        tx = xs(fnum) + cropX(1); 
        tlx = lxs(fnum) + cropX(1); 
        ty = ys(fnum) + cropY(1);
        tly = lys(fnum) + cropY(1);
        plot([tx, tlx], [ty tly], '-', 'LineWidth', 2, 'Color', [.7 .7 .7])
        % scatter([tx, tlx], [ty tly], 'ko', 'MarkerFaceColor', 'k')

        % plot(xs(1:fnum)+cropX(1), ys(1:fnum)+cropY(1), '-', 'Color', [.8 .7 .9])
        plot(xs(previnds)+cropX(1), ys(previnds)+cropY(1), '-', 'Color', [.0 .0 .0], 'LineWidth', 2)
        scatter(lxs(previnds)+cropX(1), lys(previnds)+cropY(1), 30, clr, '.')
        ...
        %%%%%%%%%%
%         im = zeros(searchsize*2+1, searchsize*2+1);
%         ledfound = meshgrid(hbad, wbad);
%         led_sz = meshgrid(hh, ww);
%         imb = imL(hh(hbad), :);
%         imb = imb(:, ww(wbad));
%         im(ledfound) = imb;
%         cla
...
% TS_data = readtable(msTSFile, 'VariableNamingRule', 'preserve');
TS_data = readmatrix(msTSFile, 'NumHeaderLines', 1);
ms = [];

recording_dir(strfind(recording_dir, '\')) = '/';

ms.parentDir = recording_dir;
ms.frameNum = TS_data(:,1); % .("Frame Number");
ms.timestamps = TS_data(:,2); % .("Time Stamp (ms)");
