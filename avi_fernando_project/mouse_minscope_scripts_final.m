function extract_mouse_pos_MovGrid(topdir, saveName)
...
vars2save = {'beh', 'topdir', 'mouse', 'led', 'barpos', 'cropX', 'cropY'};
%%
vid = VideoReader( [topdir num2str(vids(1)) '.avi']);
Yr = uint8(zeros(vid.height, vid.width, numVids*vid.NumFrames));
Yg = uint8(zeros(vid.height, vid.width, numVids*vid.NumFrames));
Yb = uint8(zeros(vid.height, vid.width, numVids*vid.NumFrames));
step = 1;
for i = 1:numVids
...
bar_thresh = 230; % pixel value thresh for bar on frame mean (rgb)
...
tic
step = 1;
plotstep = 1:10:numVids*1000;
plotstepreport = 1:1000:numVids*1000 - 1;
